...
const metrics = [
  ...
  },
  {
    ...
    href: "/customer", // Menampilkan detail konsumen sesuai permintaan
  },
  {
    title: "Disetujui",
    value: "856",
  ...
  return (
    <div className="flex items-end gap-[3px] h-8" aria-hidden="true">
      {bars.map((h, i) => (
        <div
          key={i}
          className={cn("w-[3px] rounded-full animate-bar-grow", color)}
          style={{
            height: `${h}%`,
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {metrics.map((metric, index) => (
        <Link
          key={metric.title}
          href={metric.href}
          className="group relative block overflow-hidden rounded-2xl bg-card p-5 shadow-sm ring-1 ring-border/30 transition-all duration-500 hover:shadow-lg hover:shadow-primary/[0.04] hover:-translate-y-0.5 animate-fade-in-up"
        ...
        >
          ...
          <div className="relative">
            <div className="flex items-start justify-between">
              ...
              >
                <metric.icon
                  className={cn("size-5", metric.iconColor)}
                  strokeWidth={2}
                />
              </div>
              {/* Mini spark */}
