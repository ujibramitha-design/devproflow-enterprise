    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {summaryItems.map((item, index) => (
        <Link
          key={item.label}
          href={item.href}
          className="group relative flex items-center gap-4 rounded-2xl bg-card p-4 shadow-sm ring-1 ring-border/30 transition-all duration-300 hover:shadow-md hover:ring-primary/20 animate-scale-in overflow-hidden"
          style={{ animationDelay: `${index * 80}ms` }}
        >
          {/* Hover background effect */}
          <div className={`absolute inset-0 bg-gradient-to-br ${item.accentClass} opacity-0 transition-opacity duration-300 group-hover:opacity-[0.03]`} />
